...
import { authenticateUser, DEMO_PASSWORD, getDemoUser, logoutUser, subscribeToAuth } from "@/lib/auth";
...
export default function Home() {
  const persistedRole = useSyncExternalStore(emptySubscribe, storedRole, emptySnapshot); const persistedData = useSyncExternalStore(emptySubscribe, storedData, emptyDataSnapshot); const persistedGuided = useSyncExternalStore(emptySubscribe, storedGuided, emptyGuidedSnapshot); const persistedLanguage = useSyncExternalStore(emptySubscribe, storedLanguage, emptyLanguageSnapshot); const [roleOverride, setRoleOverride] = useState(null); const [dataOverride, setDataOverride] = useState(null); const [guidedOverride, setGuidedOverride] = useState(null); const [languageOverride, setLanguageOverride] = useState(null); const [view, setView] = useState("overview"); const [notice, setNotice] = useState(""); const [busy, setBusy] = useState(false); const role = firebaseEnabled ? roleOverride : roleOverride ?? persistedRole; const guided = guidedOverride ?? persistedGuided; const language = languageOverride ?? persistedLanguage; const data = dataOverride ?? normalizeData(persistedData ? JSON.parse(persistedData) : clone());
  useEffect(() => subscribeToAuth((user) => { if (user) { window.localStorage.setItem("skillconnect-auth-mode", "firebase"); setRoleOverride(user.role); } else if (window.localStorage.getItem("skillconnect-auth-mode") === "demo") { setRoleOverride(storedRole()); } else { window.localStorage.removeItem("skillconnect-role"); setRoleOverride(null); } }), []);
  useEffect(() => { if (firebaseEnabled && !role) return undefined; let active = true; loadRemoteData().then((remote) => { if (active && remote) setDataOverride((current) => normalizeData({ ...(current || clone()), ...remote })); }).catch(() => { if (active) setNotice("Firestore unavailable; using the local demo dataset"); }); return () => { active = false; }; }, [role]);
  ...
  const selectRole = (nextUser, source = "demo") => { window.localStorage.setItem("skillconnect-auth-mode", source); setRoleOverride(nextUser?.role || ""); setView("overview"); };
