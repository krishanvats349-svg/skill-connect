...
import { titleKeys, translate, useLanguage } from "@/lib/translations";
...
function PageHeader({ title, description, action }) { const { language } = useLanguage(); const titleKey = titleKeys[title]; return <div className="page-header"><div><div className="eyebrow">Government intelligence / decision support</div><h1>{titleKey ? translate(language, titleKey) : title}</h1><p>{description}</p></div>{action}</div>; }
...
const snapshotKey = "executiveMetricsSnapshot";
const metricLabels = { openings: "openings", employers: "employers", highDemandSkills: "high-demand skills", districtsWithGaps: "districts with gaps", availableSeats: "available seats", capacityGap: "capacity gap", courseAlignment: "course alignment", trainerReadiness: "trainer readiness", candidates: "candidates", placements: "placements", feedback: "feedback records" };
function formatMetricDelta(key, delta, previous) { const sign = delta > 0 ? "+" : ""; const unit = metricLabels[key] || key; if (previous === 0) return `${sign}${delta} ${unit}`; const percentage = Math.round((delta / previous) * 1000) / 10; return `${sign}${percentage}% (${sign}${delta} ${unit})`; }
function WhatChanged({ data }) { const demand = demandRows(data); const critical = skillIntelligenceRows(data).filter((row) => row.status === "Critical Gap").length; const district = data.consultations[0]?.district || "the active district"; const [comparison, setComparison] = useState(null); const handled = useRef(false); useEffect(() => { if (handled.current || typeof window === "undefined") return; handled.current = true; const current = executiveMetrics(data); let previous = null; try { const stored = window.localStorage.getItem(snapshotKey); previous = stored ? JSON.parse(stored) : null; } catch { previous = null; } const deltas = previous ? Object.entries(current).reduce((result, [key, value]) => { if (typeof value === "number" && typeof previous[key] === "number") result[key] = { delta: value - previous[key], previous: previous[key] }; return result; }, {}) : null; setComparison({ hasPrevious: Boolean(previous), deltas }); window.localStorage.setItem(snapshotKey, JSON.stringify(current)); }, [data]); const tracked = ["openings", "highDemandSkills", "districtsWithGaps", "placements"]; return <Card className="changed-card"><div className="eyebrow">WHAT CHANGED?</div><h2>Current-state signal</h2><p>Active employer demand recorded across <b>{data.districts?.length || 3} districts</b> (led by {district}).</p>{comparison?.hasPrevious ? <div className="change-list">{tracked.map((key) => { const item = comparison.deltas[key]; return <span key={key}><b>{item ? formatMetricDelta(key, item.delta, item.previous) : "New"}</b> {item ? "since last session" : metricLabels[key]}</span>; })}</div> : <div className="change-list"><span><b>No previous snapshot available</b></span></div>}<small>{comparison?.hasPrevious ? "Compared with the executive metrics snapshot from the previous session." : "A baseline snapshot was saved for the next session."}</small></Card>; }
